...
<template>
    <view class="content">
        ...
        <view class="login-body">
            ...
            <button class="cu-btn bg-yellow big btnlogin" v-if="canIUse && !token && submitLoading" @tap="getLogin">微信授权</button>
            <button class="cu-btn bg-yellow big btnlogin" v-else-if="canIUse && token && submitLoading" open-type="getPhoneNumber" @getphonenumber="getLogin">
                绑定微信手机号
            </button>
        </view>
...
export default {
    ...
    methods: {
        loginRequest() {
            uni.login({
                success: res => {
                    this.dd('loginRequest => 初始化点击', res);
        ...
        checkSessionAndLogin(that) {
            const thisOpenId = uni.getStorageSync('token');
            // 已经进行了登录，检查登录是否过期
            if (thisOpenId) {
                that.dd('有token直接登陆');
                uni.checkSession({
        ...
        async loginAndGetOpenid() {
            ...
            const result = await that.$tools.requests(that, that.routes.api_wxlogin, paramForm, 'post');
