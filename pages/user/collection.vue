...
export default {
    ...
    methods: {
        async getData(that, page, type) {
            ...
            const result = await that.Requests(that, that.routes.api_collectList, parmform, 'post');
        ...
        async deleteBtn() {
            ...
            const array = [];
            const result = await that.Requests(that, that.routes.api_collect, parmform, 'post');
