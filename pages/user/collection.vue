...
export default {
    ...
    methods: {
        async getData(that, page, type) {
            ...
            const result = await that.$tools.requests(that, that.routes.api_collectList, parmform, 'post');
        ...
        selectAll() {
            ...
            this.parmdata.taskGrid.forEach((item) => {
        ...
        async deleteBtn() {
            const that = this
            const {selectendId} = that.parmloca;
            ...
            const array = [];
            const result = await that.$tools.requests(that, that.routes.api_collect, parmform, 'post');
            that.dd('删除的收藏请求', result);
            if (result.code === 0) {
                for (let i = 0; i < that.parmdata.taskGrid.length; i++) {
                    const item = that.parmdata.taskGrid[i];
